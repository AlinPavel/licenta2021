...
import com.licenta.service.CatalogService;
...
/**
 * REST controller for managing {@link com.licenta.domain.Catalog}.
 */
@RestController
@RequestMapping("/api")
public class CatalogResource {
    ...
    public ResponseEntity<CatalogDTO> createCatalog(@RequestBody CatalogDTO catalogDTO) throws URISyntaxException {
        ...
        if (catalogDTO.getIdCatalog() != null) {
        ...
        return ResponseEntity
            .created(new URI("/api/catalogs/" + result.getIdCatalog()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, result.getIdCatalog().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /catalogs/:idCatalog} : Updates an existing catalog.
     *
     * @param idCatalog the id of the catalogDTO to save.
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/catalogs/{idCatalog}")
    public ResponseEntity<CatalogDTO> updateCatalog(
        @PathVariable(value = "idCatalog", required = false) final Long idCatalog,
        @RequestBody CatalogDTO catalogDTO
    ) throws URISyntaxException {
        log.debug("REST request to update Catalog : {}, {}", idCatalog, catalogDTO);
        if (catalogDTO.getIdCatalog() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(idCatalog, catalogDTO.getIdCatalog())) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, catalogDTO.getIdCatalog().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /catalogs/:idCatalog} : Partial updates given fields of an existing catalog, field will ignore if it is null
     *
     * @param idCatalog the id of the catalogDTO to save.
     * @param catalogDTO the catalogDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated catalogDTO,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/catalogs/{idCatalog}", consumes = "application/merge-patch+json")
    public ResponseEntity<CatalogDTO> partialUpdateCatalog(
        @PathVariable(value = "idCatalog", required = false) final Long idCatalog,
        @RequestBody CatalogDTO catalogDTO
    ) throws URISyntaxException {
        log.debug("REST request to partial update Catalog partially : {}, {}", idCatalog, catalogDTO);
        if (catalogDTO.getIdCatalog() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(idCatalog, catalogDTO.getIdCatalog())) {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, catalogDTO.getIdCatalog().toString())
        );
    }
     ...
     */
    @GetMapping("/catalogs")
    public ResponseEntity<List<CatalogDTO>> getAllCatalogs(Pageable pageable) {
